...
def test_rag_service_production_loads_existing_persisted_index(tmp_path):
    knowledge_dir = tmp_path / "knowledge"
    knowledge_dir.mkdir()
    (knowledge_dir / "warehouse.md").write_text(
        "# 仓储场景\n仓储物流适合优先自动化与拣选辅助。",
    ...
    RAGService(settings, force_rebuild=True)
    loaded = RAGService(settings)

    assert loaded.index_status == "loaded"


def test_rag_service_production_requires_prebuilt_current_index(tmp_path):
    knowledge_dir = tmp_path / "knowledge"
    knowledge_dir.mkdir()
    document_path = knowledge_dir / "warehouse.md"
    document_path.write_text(
    ...
    with pytest.raises(RAGServiceError, match="生产环境仅加载已构建的向量索引"):
        RAGService(settings)
